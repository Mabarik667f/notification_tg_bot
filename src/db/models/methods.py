    ...
    try:
        with db.cursor() as cursor:
            ...
            if isinstance(callback_data, WeekDaysFactory):
                ...
                for d in days:
                    cursor.execute('SELECT id FROM dayofweek '
                                   'WHERE day_name = %s ', (d,))
                    id_days.append(*cursor.fetchone())

                for _id in id_days:
                    cursor.execute('INSERT INTO WeekDayHasNotification (week_day_id, notification_id) '
                ...
                sql_exact = 'INSERT INTO ExactDate (date, notification_id) VALUES (%s, %s)'
...
async def get_all_exact_notification_from_db(user_id):
    ...
    try:
        with db.cursor() as cursor:
            cursor.execute('SELECT exactdate.date, notification.notification_time, notification.text, notification.id '
                           'FROM notification '
                           'INNER JOIN exactdate ON notification.id = exactdate.notification_id '
                           'WHERE user_id = %s', (user_id,))
    ...
    try:
        with db.cursor() as cursor:
            ...
            for note in notifications:
                cursor.execute('SELECT day_name FROM dayofweek '
                               'WHERE dayofweek.id IN ( '
                               'SELECT week_day_id FROM weekdayhasnotification '
                               'WHERE notification_id = %s) '
                               'ORDER BY dayofweek.id;', (note[-1]))
                days[note[-1]] = []
